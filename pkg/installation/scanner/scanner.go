...
func findPluginManifestFiles(indexDir string) ([]string, error) {
	...
	for _, file := range files {
		if file.Type().IsRegular() && filepath.Ext(file.Name()) == global.ManifestExtension {
...
func LoadPluginByName(pluginsDir, pluginName string) (index.Plugin, error) {
	...
	return ReadPluginFromFile(filepath.Join(pluginsDir, pluginName+global.ManifestExtension))
}
