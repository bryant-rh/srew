...
import (
	...
	"github.com/bryant-rh/srew/pkg/download"
	...
	"github.com/bryant-rh/srew/pkg/pathutil"
)
...
func install(op installOperation, opts InstallOpts) error {
	...
	downloadStagingDir, err := os.MkdirTemp("", "srew-downloads")
	if err != nil {
...
func Uninstall(p environment.Paths, name string) error {
	if name == global.SrewPluginName {
		klog.Errorf("Removing srew through srew is not supported.")
		if !IsWindows() { // assume POSIX-like
			klog.Errorf("If you’d like to uninstall srew altogether, run:\n\trm -rf -- %q", p.BasePath())
		}
...
func CleanupStaleSrewInstallations(dir, currentVersion string) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to read srew store directory")
	}
	klog.V(2).Infof("Found %d entries in srew store directory", len(ls))
	for _, d := range ls {
		klog.V(2).Infof("Found a srew installation: %s (%s)", d.Name(), d.Type())
		if d.IsDir() && d.Name() != currentVersion {
			klog.V(1).Infof("Deleting stale srew install directory: %s", d.Name())
			p := filepath.Join(dir, d.Name())
			if err := os.RemoveAll(p); err != nil {
				return errors.Wrapf(err, "failed to remove stale srew version at path '%s'", p)
			}
