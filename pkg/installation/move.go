...
func findMoveTargets(fromDir, toDir string, fo index.FileOperation) ([]move, error) {
	...
	klog.V(4).Infof("Wasn't a single file, proceeding with Glob move")
...
func moveFiles(fromDir, toDir string, fo index.FileOperation) error {
	klog.V(4).Info("Finding move targets from %q to %q with file operation=%#v", fromDir, toDir, fo)
	...
	for _, m := range moves {
		klog.V(2).Info("Move file from %q to %q", m.from, m.to)
	...
	klog.V(2).Infof("Move operations are complete")
	return nil
}
...
func moveToInstallDir(srcDir, installDir string, fos []index.FileOperation) error {
	...
	klog.V(2).Infof("Creating directory %q", installationDir)
	if err := os.MkdirAll(installationDir, 0o755); err != nil {
		return errors.Wrapf(err, "error creating directory at %q", installationDir)
	}

	tmp, err := os.MkdirTemp("", "srew-temp-move")
	klog.V(2).Infof("Creating temp plugin move operations dir %q", tmp)
	if err != nil {
	...
	if err = renameOrCopy(tmp, installDir); err != nil {
		defer func() {
			klog.V(3).Infof("Cleaning up installation directory due to error during copying files")
