...
func Upgrade(p environment.Paths, plugin index.Plugin) error {
	...
	if err = receipt.Store(receipt.New(plugin, installReceipt.CreationTimestamp), p.PluginInstallReceiptPath(plugin.Name)); err != nil {
...
func cleanupInstallation(p environment.Paths, plugin index.Plugin, oldVersion string) error {
	if plugin.Name == global.SrewPluginName && IsWindows() {
