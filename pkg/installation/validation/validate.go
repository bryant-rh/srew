...
import (
	...
	"github.com/bryant-rh/srew/pkg/installation/semver"
)
...
func isSupportedAPIVersion(apiVersion string) bool {
	return apiVersion == global.CurrentAPIVersion
}
...
func ValidatePlugin(name string, p index.Plugin) error {
	if !isSupportedAPIVersion(p.APIVersion) {
		return errors.Errorf("plugin manifest has apiVersion=%q, not supported in this version of srew (try updating plugin index or install a newer version of srew)", p.APIVersion)
	}

	if p.Kind != global.PluginKind {
		return errors.Errorf("plugin manifest has kind=%q, but only %q is supported", p.Kind, global.PluginKind)
	}
